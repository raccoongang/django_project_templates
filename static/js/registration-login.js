$(document).ready(function() {
  ...
  // loading templates login
  $('[data-modal-button-login]').on('click', function() {
    $.ajax({
      url: $blockFormLogin.data('url'),
      dataType: 'html',
      success: function (data) {
        $blockFormLogin.html(data);
        $("[data-form-login]").on('submit', login);
        changeSocialUrls();
      }
    });
  });
  ...
  // login
  $("[data-form-login-submit]").on('click', login);

  function login(ev){
    ev.preventDefault();
    var urlQuery = parseUrlQuery();
    var loginRedirectUrl = '/accounts/';

    if (urlQuery.next){
      loginRedirectUrl = urlQuery.next;
    }

    var form = $blockFormLogin.find('form');
    $.ajax({
      url: $blockFormLogin.data('url'),
      dataType: 'html',
      data: form.serializeArray(),
      type: 'POST',
      success: function (data) {
        $("[data-form-login]").off('submit', login);
        $blockFormLogin.html(data);
        $("[data-form-login]").on('submit', login);
        if($blockFormLogin.find('form').length == 0){
